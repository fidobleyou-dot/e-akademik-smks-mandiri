...
function injectStudentUI(){
  ...
  box.innerHTML=`
    ...
    <div class="actions">
      <button class="success" id="submitProfileRequest">📨 Ajukan Perubahan Data Diri</button>
      <button class="secondary" id="changePasswordBtn">🔑 Ubah Password</button>
    </div>
    <p style="font-size:12px;color:#64748b">Nilai, kehadiran, perilaku, prestasi, dan data akademik tidak dapat diubah oleh siswa.</p>
    <div id="studentRequestStatus" style="margin-top:10px"></div>
  `;
  box.querySelector("#submitProfileRequest").onclick=submitProfileRequest;
...
async function submitProfileRequest(){
  const s=window.db?.students?.[0]; if(!s||!onlineUser) return;
  const payload={
    uid:onlineUser.uid, studentId:String(s.id), status:"menunggu",
  ...
  await setDoc(doc(fs,"profileRequests",`${onlineUser.uid}_${Date.now()}`),payload);
  const el=document.getElementById("studentRequestStatus");
  if(el) el.innerHTML='<div class="notice success">Pengajuan berhasil dikirim. Menunggu persetujuan Admin/Wali Kelas.</div>';
}
...
function applyStaffUI(){
  ...
  if(title) title.textContent=`e-Akademik — ${onlineProfile.role}`;
}
